...
to spreading  ;; each agent modifies with some probability its state considering the points of view (states) of its neighbors;
  ...
  ask turtles with [state = "S" or state = "B"][
    ...
    let g 0
    if den != 0 [
      ; NOTE (Erwin): Added susceptibility factor for spreading rates
      set f beta-spreadingRate * ( nB * _1PlusA / den ) * susceptibility
      set g beta-spreadingRate * ( nF * _1MinusA / den ) * (1.0 - susceptibility)
    ]
...
to veryfing  ;; B-> F ; each agent can fact-check the hoax with a fixed probability pverify;
  ask turtles with [state = "B"][
    ...
    let r random-float 1
    if susceptibility < max-susceptibility-to-verify and r < (pVerify * (1.0 - susceptibility)) [
      set state "F"
    ]
  ]
end
